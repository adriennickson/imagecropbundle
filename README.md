# Overview
The ModalBundle is a bundle for Symfony 3.0 which simplifies the process of displaying content in a modal.It provides several functionalities, from displaying simple messages and confirmation modals to displaying hole forms inside a modal.
...
```json
    "require": {
		...,
		"pitech/modal-bundle": "dev-master"
    },
    "repositories": [
        {
            "type": "vcs",
            "url": "git@gitlab.pitechplus.com:rares.serban/ModalBundle.git"
        }
    ]
```
...
```yml
pitechmodalbundle:
    resource: "@PitechModalBundle/Controller/"
    type:     annotation
```
...
$bundles = [
	...,
	new Pitech\ModalBundle\PitechModalBundle(),
];
```
...
```twig
<script src="{{ asset('bundles/pitechmodal/js/scripts.js') }}"></script>
```
...
```twig
<link href="{{ asset('bundles/pitechmodal/css/ajax-progress.css') }}" rel="stylesheet">
```

# Features
The ModalBundle provides three basic modal types: a content modal, a confirmation modal and a form modal.We will take a look at all the different modal types and how to use them.In this repository you can find on the demo branch working examples of all the things discussed here.

Keep in mind that this bundle only supports one modal open at a type, because this is the default Bootstrap behaviour.
...
This bundle can also be integrated with the Dynamic Page Bundle, but make sure that you include the javascript file from this bundle before.

### Content Modal
The content modal is the basic modal type which can be used to display pretty much any content inside a modal except for non-AJAX forms.
...
```php
     ...
     *  class: modal-open
     *
     * @Route("/modal/open/{message}", name="modal_open_message")
     */
    public function openMessageModalAction($message)
    {
        return $this->render('PitechModalBundle::messageModal.html.twig', [
...
```twig
<button href="{{ path('modal_open_message', {'message': 'info.message'}) }}" class="modal-open" type="button" class="btn btn-secondary">Open Test Modal</button>
```
...
```php
     ...
     *  data-modal-href: {{ path('modal_open_confirm') }}
     *
     * @Route("/modal/confirm", name="modal_open_confirm")
     */
    public function openConfirmModalAction()
    {
        return $this->render('PitechModalBundle::baseConfirmModal.html.twig');
    }
```
...
```twig
<a href="{{ path('contact_index') }}" class="modal-open-confirm" data-modal-href="{{ path('modal_open_confirm') }}">Open Confirm Modal</a>
```
...
return $this->createFormBuilder()
            ->add('delete', SubmitType::class, [
                'attr' => [
                    'class' => 'modal-open-confirm',
                    'data-modal-href' => $this->generateUrl('modal_open_confirm'),
                ],
            ])
...
### Form Modal
The form modal is the hardest modal to implement compared to the others, but it is still pretty easy to use.

To open a modal that contains a form, you need to add the class **modal-open-form** to a clickable element that also needs to have the href attribute set to a controller action that returns a modal template with a form inside.The **data-modal-id** attribute is also supported on this form type.

```twig
<a class="modal-open-form" href="{{ path('contact_edit_modal', { 'id': contactMessage.id }) }}">edit modal</a>
```

The controller action that returns the form needs to be slightly modified also.The action url will need to be explicitly set on the form, preferably to the same controller action.Also, when the form is successfully submitted, the redirection to a new page should be done by returning a new [ModalRedirectResponse](Response/ModalRedirectResponse.php) object, which takes an url as a parameter.
